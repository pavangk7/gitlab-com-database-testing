...
ssh -f -N -L ${DBLAB_API_PORT}:localhost:2345 -i ~/.ssh/id_rsa "${DBLAB_SSH_HOST}"

dblab init --url http://127.0.0.1:${DBLAB_API_PORT} --token "${DBLAB_TOKEN}" --environment-id "${DBLAB_ENVIRONMENT}"
